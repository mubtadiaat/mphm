...
Integritasi Data Sebelum & Sesudah: Untuk aksi PUT (Update), middleware melakukan operasi baca cepat (pre-fetch read) data yang ada di database sebelum perubahan dieksekusi, lalu mencatatnya ke dalam kolom beforeData. Setelah query simpan berhasil, payload baru dimasukkan ke kolom afterData. Mekanisme ini memastikan rekam jejak forensik data jika terjadi kesalahan administratif atau intervensi nilai ilegal.

🌟 MASTER BLUEPRINT MPHM v4.0 (ULTIMATE EDITION)
#08: OTORISASI HAK AKSES (RBAC), KEAMANAN, & GLOBAL REALTIME AUDIT TRAIL
Keamanan, otorisasi, dan forensik data pada ekosistem MPHM v4.0 diimplementasikan secara terpusat di tingkat lapisan Backend API Gateway (Hono.js di atas Vercel Edge/Serverless). Kebijakan ini memastikan bahwa seluruh operasi baca dan tulis (read/write) divalidasi pada jangkauan Edge Server sebelum mengeksekusi perintah ke basis data Neon Postgres, memberikan perlindungan berlapis dengan performa tinggi.
...
Setiap baris log audit yang disimpan di dalam tabel audit_logs Neon Postgres wajib memuat data berikut:
...
- Universal Bulk & Soft Delete: Aksi massal menggunakan `POST /bulk/update`, `POST /bulk/delete`, dan `POST /bulk/restore`. Penghapusan wajib menggunakan prinsip *Soft Delete* (Recycle Bin) dan dilindungi oleh *Database Transaction* pada tingkat Neon Postgres.
