...
violationsAdmin.get("/categories", async (c) => {
  const db = createDb();
  const list = await db.select().from(violationCategories).where(eq(violationCategories.isActive, true));
...
violationsAdmin.get("/severities", async (c) => {
  const db = createDb();
  const list = await db.select().from(violationSeverities).where(eq(violationSeverities.isActive, true));
  return c.json({ status: "Success", data: list });
});
