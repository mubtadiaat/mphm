...
onboardingAdmin.get("/status", async (c) => {
  try {
    ...
    // 1. Check Mundzir
    const mundzirCount = await db.run(sql`
    ...
    // 2. Check Mufattisy
    const mufattisyCount = await db.run(sql`
      SELECT count(*) as count 
      FROM organization_memberships 
    ...
    const mustahiqCount = await db.run(sql`
      SELECT count(*) as count 
    ...
    // 4. Check Classes
    const classesCount = await db.run(sql`
      SELECT count(*) as count 
    ...
    // 5. Check Santri
    const santriCount = await db.run(sql`
      SELECT count(*) as count 
      FROM student_profiles 
      WHERE deleted_at IS NULL
    `);

    const getCount = (result: any) => ((result.results?.[0] as { count?: number })?.count || 0) > 0;
